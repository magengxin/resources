...
//          formatter: "{b} : {d}%" //提示框浮层内容格式器
    ...
    echarts.init(document.getElementById('chart_110kv_pie')).setOption(option);//110kv
...
//      color: ['#0992f2', '#1efc3d', '#1ee6fd', '#1e43fc'],  //手动设置每个图例的颜色
    ...
    echarts.init(document.getElementById('chart_35kv_pie')).setOption(option);//35kv
}
/**
 * 全网状态 
 * 下 
...
function chart_10kv_pie() {
    ...
    echarts.init(document.getElementById('chart_10kv_pie')).setOption(option);//10kv
}
/**
 * 全网状态 
 * 下 
...
function chart_04kv_pie() {
    ...
    echarts.init(document.getElementById('chart_04kv_pie')).setOption(option);//0.4kv
}
/**
 * 全网状态 
 * 下 
...
function chart_gzfb_bar() {
   ...
   echarts.init(document.getElementById('chart_gzfb_bar')).on("click",function clickEchart(param) {
            ...
            $('#qiangdan_title',parent.document).text("故障监测");//明细列表标题
  });
    
}
/**
 * 
 * 故障监测明细:按电压等级 公司 与 是否核心区
