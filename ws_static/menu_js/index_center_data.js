...
$(function () {
    getWenDuData();//温度接口
    getFuHeData();//负荷接口
    getYuJingZTBDData();//预警状态接口
    setInterval(getWenDuData,1000*60*30);//三分钟刷新温度
    setInterval(getFuHeData,1000*60);//一分钟刷新负荷
    loadXX();//走马灯
})

function loadXX() {
    showZouMaDeng(g_deptchange);//走马灯
}
...
function getFuHeData(){
    ...
    function getFuHeList(SSvalue) {//获取负荷详情
        var date = gettime();
